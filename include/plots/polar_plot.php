...
FORM;
  ...
  echo '      <button id="polarFlagsToggleBtn" onclick="togglePolarSingleFlags(this)">Hide Flags</button>';
  echo '    </div>';
  echo '  </details>';
  ...
  echo "\n";
  echo "  if (typeof window.__polarSingleFlagsVisible === 'undefined') window.__polarSingleFlagsVisible = true;\n";
  echo "\n";
  echo "  window.applyPolarFlagsVisibility = function(visible) {\n";
  ...
  echo "    window.applyPolarFlagsVisibility(next);\n";
  echo "  };\n";
  echo "\n";
  ...
  echo "\n";
  echo "  window.applyPolarFlagsVisibility(window.__polarSingleFlagsVisible !== false);\n";
  echo "\n";
...
<div id="polarZoomModal" style="
  ...
  echo "  if (window.__polarZoomBrushCleanup) {\n";
  echo "    window.__polarZoomBrushCleanup();\n";
  echo "    window.__polarZoomBrushCleanup = null;\n";
  echo "  }\n";
  ...
  echo "  var zoomLayer = g.append('g');\n";
  echo "  var currentTransform = d3.zoomIdentity;\n";
  echo "  var timeBisect = d3.bisector(function(d) { return d.time; }).left;\n";
  echo "  var brushOrigin = null;\n";
  echo "  var brushRect = null;\n";
  echo "\n";
  echo "  var renderTooltip = function(event, d) {\n";
  ...
  echo "  };\n";
  echo "\n";
  echo "  var zb = d3.zoom().scaleExtent([0.5, 12]).filter(function(event) { return !event.shiftKey; })\n";
  echo "    .on('start', function() { tooltip.style('opacity', 0); })\n";
  echo "    .on('zoom', function(event) {\n";
  echo "      currentTransform = event.transform;\n";
  echo "      zoomLayer.attr('transform', event.transform);\n";
  echo "    });\n";
  echo "  svg.call(zb);\n";
  echo "\n";
  echo "  brushRect = svg.append('rect')\n";
  echo "    .attr('class', 'polar-zoom-brush-rect')\n";
  echo "    .style('fill', 'rgba(70, 130, 180, 0.12)')\n";
  echo "    .style('stroke', '#4682b4')\n";
  echo "    .style('stroke-width', '1.5px')\n";
  echo "    .style('stroke-dasharray', '5,3')\n";
  echo "    .style('pointer-events', 'none')\n";
  ...
  echo "    brushRect.attr('x', pointer[0]).attr('y', pointer[1]).attr('width', 0).attr('height', 0).style('display', null);\n";
  echo "  });\n";
  echo "\n";
  echo "  svg.on('mousemove.polarZoomBrush', function(event) {\n";
  echo "    if (!brushOrigin) return;\n";
  echo "    svg.style('cursor', 'crosshair');\n";
  echo "    var pointer = d3.pointer(event, svg.node());\n";
  echo "    var bx = Math.max(0, Math.min(width, pointer[0]));\n";
  echo "    var by = Math.max(0, Math.min(height, pointer[1]));\n";
  echo "    brushRect.attr('x', Math.min(brushOrigin[0], bx)).attr('y', Math.min(brushOrigin[1], by)).attr('width', Math.abs(bx - brushOrigin[0])).attr('height', Math.abs(by - brushOrigin[1]));\n";
  echo "  });\n";
  echo "\n";
  echo "  var finishBrush = function(event) {\n";
  echo "    if (!brushOrigin) return;\n";
  echo "    brushRect.style('display', 'none');\n";
  echo "    svg.style('cursor', null);\n";
  echo "    var pointer = event ? d3.pointer(event, svg.node()) : [brushOrigin[0], brushOrigin[1]];\n";
  echo "    var bx = Math.max(0, Math.min(width, pointer[0]));\n";
  echo "    var by = Math.max(0, Math.min(height, pointer[1]));\n";
  echo "    var x0 = Math.min(brushOrigin[0], bx);\n";
  echo "    var x1 = Math.max(brushOrigin[0], bx);\n";
  echo "    var y0 = Math.min(brushOrigin[1], by);\n";
  echo "    var y1 = Math.max(brushOrigin[1], by);\n";
  echo "    brushOrigin = null;\n";
  echo "    if ((x1 - x0) < 8 || (y1 - y0) < 8) return;\n";
  echo "    var local0 = currentTransform.invert([x0 - cx, y0 - cy]);\n";
  echo "    var local1 = currentTransform.invert([x1 - cx, y1 - cy]);\n";
  echo "    var ux0 = Math.min(local0[0], local1[0]);\n";
  echo "    var ux1 = Math.max(local0[0], local1[0]);\n";
  echo "    var uy0 = Math.min(local0[1], local1[1]);\n";
  echo "    var uy1 = Math.max(local0[1], local1[1]);\n";
  echo "    var boxWidth = ux1 - ux0;\n";
  echo "    var boxHeight = uy1 - uy0;\n";
  echo "    if (boxWidth <= 0 || boxHeight <= 0) return;\n";
  echo "    var targetSpan = Math.max(outerRadius * 2 - 20, 50);\n";
  echo "    var newK = Math.min(12, Math.max(0.5, Math.min(targetSpan / boxWidth, targetSpan / boxHeight)));\n";
  echo "    var centerX = (ux0 + ux1) / 2;\n";
  echo "    var centerY = (uy0 + uy1) / 2;\n";
  echo "    var newTransform = d3.zoomIdentity.translate(-newK * centerX, -newK * centerY).scale(newK);\n";
  echo "    svg.transition().duration(250).call(zb.transform, newTransform);\n";
  echo "  };\n";
  echo "\n";
  echo "  svg.on('mouseup.polarZoomBrush', finishBrush);\n";
  echo "\n";
  echo "  var onWindowMouseUp = function(event) {\n";
  echo "    if (!brushOrigin) return;\n";
  echo "    finishBrush(event);\n";
  echo "  };\n";
  echo "  window.addEventListener('mouseup', onWindowMouseUp);\n";
  echo "\n";
  echo "  var onKeyDown = function(event) {\n";
  echo "    if (event.key === 'Shift' && !brushOrigin) svg.style('cursor', 'crosshair');\n";
  echo "  };\n";
  echo "  var onKeyUp = function(event) {\n";
  echo "    if (event.key !== 'Shift') return;\n";
  echo "    if (brushOrigin) { brushRect.style('display', 'none'); brushOrigin = null; }\n";
  echo "    svg.style('cursor', null);\n";
  echo "  };\n";
  echo "  document.addEventListener('keydown', onKeyDown);\n";
  echo "  document.addEventListener('keyup', onKeyUp);\n";
  echo "\n";
  echo "  window.__polarZoomBrushCleanup = function() {\n";
  echo "    window.removeEventListener('mouseup', onWindowMouseUp);\n";
  echo "    document.removeEventListener('keydown', onKeyDown);\n";
  echo "    document.removeEventListener('keyup', onKeyUp);\n";
  echo "  };\n";
  echo "\n";
  echo "  window.resetPolarZoom = function() {\n";
  ...
  echo "  zoomLayer.selectAll('.pzmgc').data(timeTicks).enter().append('circle')\n";
  ...
  echo "  zoomLayer.selectAll('.pzmal').data(dirs).enter().append('line')\n";
  echo "    .attr('x1',0).attr('y1',0)\n";
  echo "    .attr('x2', function(d) { return angleToXY(d, outerRadius).x; })\n";
  echo "    .attr('y2', function(d) { return angleToXY(d, outerRadius).y; })\n";
  echo "    .attr('stroke','#8fa5ba');\n";
  echo "  zoomLayer.selectAll('.pzmlbl').data(dirs).enter().append('text')\n";
  ...
  echo "      zoomLayer.append('line').attr('x1',pa.x).attr('y1',pa.y).attr('x2',pb.x).attr('y2',pb.y)\n";
  ...
  echo "  });\n";
  echo "\n";
  echo "  zoomLayer.selectAll('.pzm-flag-point').data(flaggedPoints).enter().append('circle')\n";
  ...
  echo "      if (radius < innerRadius || radius > outerRadius) {\n";
  echo "        tooltip.style('opacity', 0);\n";
  echo "        return;\n";
  echo "      }\n";
  ...
  echo "        return;\n";
  echo "      }\n";
  echo "      renderTooltip(event, nearest);\n";
  echo "    })\n";
  ...
  echo "    zoomLayer.append('circle').attr('cx',pos.x).attr('cy',pos.y).attr('r',5.5)\n";
  echo "      .attr('fill',fill).attr('stroke','#ffffff').attr('stroke-width',1.8);\n";
  echo "    zoomLayer.append('text').attr('x',pos.x+8).attr('y',pos.y-8)\n";
